...
学生移动考勤系统是一个基于Node.js和MySQL开发的后端服务，为学生和教师提供考勤管理功能。系统支持学生通过移动端进行打卡，教师通过Web端管理考勤活动和学生信息，管理员通过Web端管理教师账号和所有考勤活动。
...
├── README.md             # 项目说明文档
├── readme1.md            # Apifox测试指南（通用）
├── readme2.md            # Apifox测试指南（角色分离）
└── node_modules/         # 依赖包
```
...
4. **创建数据库**
   ...
   CREATE TABLE `users` (
       ...
       `student_id` VARCHAR(20) UNIQUE NULL COMMENT '学号（学生必填，教师/管理员可空）',
...
3. 添加以下环境变量：
   ...
   - `admin_token`：留空，后续测试时会动态设置
   - `teacher_token`：留空，后续测试时会动态设置
   - `student_token`：留空，后续测试时会动态设置
4. 点击「保存」按钮
...
4. 点击「发送」按钮，查看响应
5. 在响应结果中，复制 `token` 值
6. 在环境变量中，将 `admin_token` 的值设置为复制的token值，点击「保存」

#### 测试教师登录
1. 首先，使用管理员账号添加一个教师账号：
   - 创建一个新的POST接口，URL为 `/api/users/teachers`
   - 请求头中添加 `Authorization: Bearer {{admin_token}}`
   - 请求体中输入：
     ```json
     {
       "username": "teacher1",
...
3. 请求体中输入：
   ...
   ```
4. 点击「发送」按钮，复制响应中的token值
5. 在环境变量中，将 `teacher_token` 的值设置为复制的token值，点击「保存」

#### 测试学生登录
1. 首先，使用教师账号添加一个学生账号：
   - 创建一个新的POST接口，URL为 `/api/users/students`
   - 请求头中添加 `Authorization: Bearer {{teacher_token}}`
   - 请求体中输入：
     ```json
     {
       "username": "student1",
...
3. 请求体中输入：
   ...
   ```
4. 点击「发送」按钮，复制响应中的token值
5. 在环境变量中，将 `student_token` 的值设置为复制的token值，点击「保存」
...
2. 填写接口信息：
   - 接口名称：「获取所有教师 - 管理员」
   - 请求方法：GET
   - 请求URL：`/api/users/teachers`
3. 在「请求头」选项卡中，添加：
   - 键：`Authorization`
   - 值：`Bearer {{admin_token}}`
4. 点击「发送」按钮，查看响应
5. 验证是否返回所有教师信息
...
2. 填写接口信息：
   - 接口名称：「管理员查看所有活动」
   - 请求方法：GET
   - 请求URL：`/api/activities/teacher`
3. 在「请求头」选项卡中，添加：
   - 键：`Authorization`
   - 值：`Bearer {{admin_token}}`
4. 点击「发送」按钮，验证是否返回所有活动
...
3. 添加以下测试步骤：
   ...
   - 提交申诉 - 学生
4. 点击「运行」按钮，执行批量测试
5. 查看测试结果，确保所有接口都正常工作，权限控制正确

## 注意事项

1. 所有需要认证的接口都需要在请求头中添加 `Authorization: Bearer <token>`
2. **管理员角色**：可以访问所有功能，包括管理教师账号和所有考勤活动
3. **教师角色**：只能访问自己的功能，包括管理学生和自己创建的考勤活动
4. **学生角色**：只能访问学生相关的功能，如打卡和查看自己的记录
5. 打卡时需要确保GPS定位在地理围栏内，否则会被标记为异常
6. 每个考勤活动，学生只能打卡一次
7. 修改密码时，新密码长度至少为6个字符
