...
const refreshSchedule = async () => {
  loading.value = true;
  try {
    const activities: Activity[] = await getStudentSchedule();
    courses.value = activities.map((a): Course => ({
      id: a.id,
      name: a.title,
      date: a.start_time.split(' ')[0],
      startTime: a.start_time.split(' ')[1] || '',
      endTime: a.end_time.split(' ')[1] || '',
