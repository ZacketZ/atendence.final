...
class RecordService {
  // 学生打卡
  async createRecord(user_id, activity_id, check_time, gps_longitude, gps_latitude, liveness_result, device_info) {
    try {
      // 检查活动是否存在
      ...
      // 创建打卡记录
      await db.query(
        'INSERT INTO attendance_records (activity_id, user_id, check_time, gps_longitude, gps_latitude, liveness_result, final_status, device_info) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [activity_id, user_id, check_time, gps_longitude, gps_latitude, liveness_result, final_status, device_info]
      );
